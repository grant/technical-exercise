  ...
  const giftIdeas = await Promise.all(
    shoppingList.map(async (person) => {
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          prompt: `Suggest 1 Christmas gift idea related to ${person.interests} for ${person.name} with a budget of $${budget}. 
          Return only a JSON object with this exact format:
          {
            "name": "item name",
            "price": number
          }
          The price must be less than or equal to ${budget}.`,
        }),
      });
